	...
	namespace MusicUI
	{
		...
		void DivaSongInfoContainer::draw(Graphics *graphics)
		{
			...
			if (textVisible)
			{
				...
				wchar_t temp[100];
				// Name
				wsprintf(temp, L"Song Name %s", mapInfo.header.name.c_str());
				graphics->drawTextW(temp, x, h * 1 + ydelta);

				// Artist
				if (mapInfo.header.artists.size() == 0)
					wsprintf(temp, L"Composer unknown");
				else
					wsprintf(temp, L"Composer %s", mapInfo.header.artists[0].c_str());
				graphics->drawTextW(temp, x, h * 2 + ydelta);

				// Lyric
				if (mapInfo.header.lyricists.size() == 0)
					wsprintf(temp, L"Lyrics unknown");
				else
					wsprintf(temp, L"Lyrics %s", mapInfo.header.lyricists[0].c_str());
				graphics->drawTextW(temp, x, h * 3 + ydelta);

				// Lyric
				if (mapInfo.header.composers.size() == 0)
					wsprintf(temp, L"Arranger unknown");
				else
					wsprintf(temp, L"Arranger %s", mapInfo.header.composers[0].c_str());
				graphics->drawTextW(temp, x, h * 4 + ydelta);

				// noter
				if (mapInfo.header.noters.size() == 0)
					wsprintf(temp, L"Noter unknown");
				else
				...
				if (mapInfo.header.vocaloids.size() == 0)
					wsprintf(temp, L"VOCALOID unknown");
				else
				{
					...
					wsprintf(temp, L"VOCALOID %s", vocaloidStr.c_str());
				}
