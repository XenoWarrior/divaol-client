...
namespace divapomelo
{
	...
	public:
		...
		void sendTo(const std::string &uid, Base::String content)
		{
			...
			request(EventCode[PUSH_CHAT_SEND],Json::Object("content", content.asUTF8(), "scope", "private","target", uid),PeerComp::defaultRequestHandler);
		}
		...
		void sendInStage(Base::String content)
		{
			...
			request(EventCode[PUSH_CHAT_SEND],Json::Object("content", content.asUTF8(),"scope", "stage"),PeerComp::defaultRequestHandler);
		}
		...
		void sendGlobal(Base::String content)
		{
			...
			request(EventCode[PUSH_CHAT_SEND],Json::Object("content", content.asUTF8(),"scope", "global"),PeerComp::defaultRequestHandler);
		}
